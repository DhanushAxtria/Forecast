...
const Patient_Forecast_Input = () => {
    ...
    const handleSaveInfo = (tabKey, tableKey) => {
        setSavedText((prev) => ({
            ...prev,
                    ...
                    <thead>
                        <tr>
                            ...
                            {columns.map((column, rowIndex) => (
                                <th key={rowIndex} style={{ minWidth: '150px' }}>{column}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {rows.map((product, rowIndex) => (
                            <tr key={`${product.id}-${product.caseType}-${rowIndex}`}>
                                <td >
                                    <div style={{
                                        display: 'grid', gridTemplateColumns: ' 0.1fr 0.1fr 1fr 0.1fr 0.1fr',
                                        ...
                                        {editingProductId === product.id ? (
                                            ...
                                            <>
                                                ...
                                                <Tooltip title="Edit Row Name" placement="top" >
                                                    <IconButton onClick={() => handleEditClick(product.id)} style={{ marginLeft: '8px' }}>
                                                        <EditIcon fontSize="small" />
                                                    </IconButton>
                                                </Tooltip>

                                                <Tooltip title="Delete Parameter" placement="top" >
                                                    <IconButton onClick={() => handleDeleteParameter(product.id)} style={{ marginLeft: '8px' }}>
                                ...
                                {columns.map((date) => (
                                    <td key={date}>
                                        <TextField
                                            type="number"
                                            value={values[rowIndex]?.[date] || ''}
                                            onChange={(e) => handleValueChange(rowIndex, date, e.target.value)}
                                            variant="outlined"
                                            size="small"
